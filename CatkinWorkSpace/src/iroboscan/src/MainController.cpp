   ...
   scanningController(nullptr)
{
	...
	//Resolution::getInstance(1080, 1080);
    ...
    else
    {
        ...
        //Intrinsics::getInstance(823.468994140625, 823.468994140625, 540, 540);
        //Intrinsics::getInstance(742.7332153320312, 742.7332153320312, 540, 540);
    }

    Parse::get().arg(argc, argv, "-l", logFile);
    Parse::get().arg(argc, argv, "-sg", simFile);	// use -sg to run in a simulated gazebo environment
    Parse::get().arg(argc, argv, "-rr", rosLiveFile); // use -rr to use ros topic as real live input

    if(logFile.length())
    {
        logReader = new RawLogReader(logFile, Parse::get().arg(argc, argv, "-f", empty) > -1);
    }
	else if (simFile.length())
	{
		logReader = new SimLogReader(simFile, Parse::get().arg(argc, argv, "-f", empty) > -1);
		good = static_cast<SimLogReader *>(logReader)->ok();
	}
	else if (rosLiveFile.length())
	{
		logReader = new ROSLiveLogReader(rosLiveFile, Parse::get().arg(argc, argv, "-f", empty) > -1);
		good = static_cast<ROSLiveLogReader *>(logReader)->ok();
	}
    else
    {
        ...
        good = static_cast<LiveLogReader *>(logReader)->cam->ok();
    ...
    end = std::numeric_limits<int>::max(); //Funny bound, since we predict times in this format really!
    ...
    while(good)
    {
        ...
        if(eFusion == 0 || resetButton)
        {
			...
			if (scanningController)
				delete scanningController;
			if (simFile.length())
				scanningController = new ScanningController(eFusion, static_cast<SimLogReader *>(logReader), gui);
			else
				scanningController = new ScanningController(eFusion, gui);
        }
        else
        {
            break;
...
void MainController::run()
{
    ...
    {
        if((!gui->pause->Get() || pangolin::Pushed(*gui->step)) && !scanningController->blockFusion())
        {
            if((logReader->hasMore() || rewind) && eFusion->getTick() < end)
            {
				...
				else if (simFile.length())
				{
					currentPose = new Eigen::Matrix4f;
					currentPose->setIdentity();
					*currentPose = static_cast<SimLogReader *>(logReader)->getCurrentPose();
				}
		...
		if (eFusion->getTick() == 100 && simFile.length())
		{
			scanningController->initializePose();
		}
		...
		else if (gui->godView->Get())
		{
			pangolin::OpenGlMatrix mv;

			Eigen::Matrix4f currPose = eFusion->getCurrPose();

			Eigen::Vector3f forward(0, 1, 0);
			Eigen::Vector3f up(0, 0, 1);
			Eigen::Vector3f eye(currPose(0, 3), currPose(1, 3) - 8, currPose(2, 3));
		...
		if (gui->drawMatchedModels->Get() && scanningController)
		{
			scanningController->renderNBOMatchingModels(gui->s_cam.GetProjectionModelViewMatrix());
		}
		...
		if (gui->drawNBO->Get() && scanningController)
		{
			glColor3f(0, 1, 1);
			scanningController->renderNBOPosition();
        	glColor3f(1, 1, 1);
		}
		if (gui->drawNBV->Get() && scanningController)
		{
			glColor3f(0, 1, 1);
			scanningController->renderNBVPose();
        	glColor3f(1, 1, 1);
		}
		if (gui->drawNBVTrajectory->Get() && scanningController)
		{
			scanningController->renderNBVTrajectory();
        	glColor3f(1, 1, 1);
		}
        ...
        if(pangolin::Pushed(*gui->savePatch))
        {
            eFusion->savePatchPly();
        }

		if (scanningController)
		{
			if (pangolin::Pushed(*gui->stepScan))
				scanningController->stepScan();

			if (gui->autoScan->Get())
				scanningController->autoScan();
			else
				scanningController->stopScan();

			if (pangolin::Pushed(*gui->nextNBV))
				scanningController->popCurrentNBV();

			scanningController->loop();
		}
