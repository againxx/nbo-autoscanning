...
static inline std::shared_ptr<Shader> loadProgramGeomFromFile(const std::string& vertex_shader_file, const std::string& geometry_shader_file)
{
	...
	std::string shaderDir = getenv("HOME");
	shaderDir += "/iroboscan/Seg/src/Shaders";

    program->AddShaderFromFile(pangolin::GlSlVertexShader, shaderDir + "/" + vertex_shader_file, {}, {shaderDir});
    program->AddShaderFromFile(pangolin::GlSlGeometryShader, shaderDir + "/" + geometry_shader_file, {}, {shaderDir});
    program->Link();
...
static inline std::shared_ptr<Shader> loadProgramFromFile(const std::string& vertex_shader_file, const std::string& fragment_shader_file)
{
    ...
    program->AddShaderFromFile(pangolin::GlSlVertexShader, shaderDir + "/" + vertex_shader_file, {}, {shaderDir});
    program->AddShaderFromFile(pangolin::GlSlFragmentShader, shaderDir + "/" + fragment_shader_file, {}, {shaderDir});
    program->Link();
