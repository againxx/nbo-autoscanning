   ...
   deformationNodes(NODE_TEXTURE_DIMENSION, 1, GL_LUMINANCE32F_ARB, GL_LUMINANCE, GL_FLOAT)
{
	...
	cudaGraphicsGLRegisterBuffer(&cudaResources[0], vbos[0].first, cudaGraphicsRegisterFlagsNone);
	cudaGraphicsGLRegisterBuffer(&cudaResources[1], vbos[1].first, cudaGraphicsRegisterFlagsNone);
                                   ...
                                   const int timeDelta)
{
    ...
    program->setUniform(Uniform("colorType", (drawNormals ? 1 : drawColors ? 2 : drawSegmentation ? 3 : drawTimes ? 4 : 0)));
...
void GlobalModel::fuse(const Eigen::Matrix4f & pose,
					   ...
					   //const int mergingPairSize,
					   //const float mergingTextureDim)
{
    TICK("Fuse::Data");
    ...
    dataProgram->setUniform(Uniform("lSampler", 3));
    ...
    glBindTexture(GL_TEXTURE_2D, label->texture->tid);
    ...
    updateProgram->setUniform(Uniform("mergeSamp", 3));
    updateProgram->setUniform(Uniform("texDim", (float)TEXTURE_DIMENSION));
    updateProgram->setUniform(Uniform("time", time));
    //updateProgram->setUniform(Uniform("mergeSize", mergingPairSize));
    //updateProgram->setUniform(Uniform("mergeTexDim", mergingTextureDim));
    ...
    //glActiveTexture(GL_TEXTURE3);
    //glBindTexture(GL_TEXTURE_2D, mergingLabels->texture->tid);
...
void GlobalModel::uploadMap(const Eigen::Vector4f * vertices)
{
	glFinish();

	GLuint uploadVbo;

	glGenBuffers(1, &uploadVbo);
	glBindBuffer(GL_ARRAY_BUFFER, uploadVbo);
	glBufferData(GL_ARRAY_BUFFER, bufferSize, 0, GL_STREAM_DRAW);
	glBindBuffer(GL_ARRAY_BUFFER, 0);

	glBindBuffer(GL_COPY_WRITE_BUFFER, vbos[target].first);
	glBindBuffer(GL_COPY_READ_BUFFER, uploadVbo);

	glBufferSubData(GL_COPY_READ_BUFFER, 0, count * Vertex::SIZE, vertices);
    glCopyBufferSubData(GL_COPY_READ_BUFFER, GL_COPY_WRITE_BUFFER, 0, 0, count * Vertex::SIZE);

    glBindBuffer(GL_COPY_READ_BUFFER, 0);
    glBindBuffer(GL_COPY_WRITE_BUFFER, 0);
    glDeleteBuffers(1, &uploadVbo);

	glFinish();
}

void GlobalModel::mergeLabelPairs(const std::vector<std::pair<unsigned short, unsigned short>> & labelPairs)
{
	float4 * mapData = getMappedPointer();

	unsigned short * labels = new unsigned short[2 * labelPairs.size()];
	for (size_t i = 0; i < labelPairs.size(); ++i)
	{
		labels[2 * i] = labelPairs[i].first;
		labels[2 * i + 1] = labelPairs[i].second;
	}

	mergePairs(mapData, count, labels, labelPairs.size());

	unmapResource();
	delete [] labels;
}

std::set<unsigned short> GlobalModel::getExistingLabels(unsigned short labelRange)
{
	float4 * mapData = getMappedPointer();
	unsigned int * labelCounter = new unsigned int[labelRange];

	collectExistingLabels(mapData, count, labelCounter, labelRange);

	unmapResource();

	std::set<unsigned short> existingLabels;
	for (unsigned short i = 1; i < labelRange; ++i)
	{
		if (labelCounter[i] > 0)
...
float4 * GlobalModel::getMappedPointer()
{
	glFinish();
	cudaGraphicsResource * mapCudaRes = cudaResources[target];

	float4 * mapData;
	cudaGraphicsMapResources(1, &mapCudaRes, 0);
	size_t byteNum;
	cudaGraphicsResourceGetMappedPointer((void **)&mapData, &byteNum, mapCudaRes);

	return mapData;
}

void GlobalModel::unmapResource()
{
	cudaGraphicsUnmapResources(1, &cudaResources[target], 0);
}
