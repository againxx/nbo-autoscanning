...
ElasticFusion::ElasticFusion(const int timeDelta,
 ...
 : frameToModel(Resolution::getInstance().width(),
   ...
   frameSegmentation(new FrameSegmentation)
{
    createTextures();
    createCompute();
...
void ElasticFusion::createTextures()
{
	...
	textures[GPUTexture::LABEL] = new GPUTexture(Resolution::getInstance().width(),
												 Resolution::getInstance().height(),
	...
	textures[GPUTexture::VERTEX_METRIC] = new GPUTexture(Resolution::getInstance().width(),
													  	 Resolution::getInstance().height(),
													  	 GL_RGB32F,
													  	 GL_RGB,
													  	 GL_FLOAT);

	textures[GPUTexture::NORMAL_METRIC_FILTERED] = new GPUTexture(Resolution::getInstance().width(),
													  			  Resolution::getInstance().height(),
													  			  GL_RGB32F,
													  			  GL_RGB,
													  			  GL_FLOAT);

	//textures[GPUTexture::MERGING_LABEL] = new GPUTexture(FrameSegmentation::maxMergingPairSize,
	//													 1,
...
void ElasticFusion::createCompute()
{
	...
	computePacks[ComputePack::VERTEX] = new ComputePack(loadProgramFromFile("empty.vert", "cal_vertex.frag", "quad.geom"),
														textures[GPUTexture::VERTEX_METRIC]->texture);

	computePacks[ComputePack::NORMAL] = new ComputePack(loadProgramFromFile("empty.vert", "cal_normal.frag", "quad.geom"),
														textures[GPUTexture::NORMAL_METRIC_FILTERED]->texture);
}
                                 ...
                                 const bool bootstrap)
{
	...
	unsigned short * zeroLabel = new unsigned short[Resolution::getInstance().numPixels()];
    memset(zeroLabel, 0, sizeof(unsigned short) * Resolution::getInstance().numPixels());
	textures[GPUTexture::LABEL]->texture->Upload(zeroLabel, GL_LUMINANCE_INTEGER_EXT, GL_UNSIGNED_SHORT);
	delete [] zeroLabel;
    ...
    else
    {
        ...
        if(!rgbOnly && trackingOk && !lost)
        {
			...
			if (tick % 5 == 0)
				computeFramewiseLabels(rgb, depth);
            ...
            globalModel.fuse(currPose,
							 ...
							 textures[GPUTexture::LABEL],
							 //textures[GPUTexture::MERGING_LABEL],
                             indexMap.indexTex(),
                             ...
                             confidenceThreshold,
                             weighting);
							 //frameSegmentation->getMergingPairs().size(),
							 //FrameSegmentation::maxMergingPairSize);
	...
	if (frameSegmentation->needMerging())
	{
		if (tick % 100 == 0 || frameSegmentation->getMergingPairs().size() == FrameSegmentation::maxMergingPairSize)
		{	
			frameSegmentation->removeGroundPlainMergingPairs(groundPlainLabels);
			globalModel.mergeLabelPairs(frameSegmentation->getMergingPairs());
			frameSegmentation->clearMergingPairs();
		}
	}

	if (tick % 200 == 0)
		frameSegmentation->recycleOverridedLabels(globalModel.getExistingLabels(frameSegmentation->getCurrentLabelRange()));
...
void ElasticFusion::computeVertexesAndNormals()
{
	std::vector<Uniform> uniforms;
	...
	computePacks[ComputePack::VERTEX]->compute(textures[GPUTexture::DEPTH_METRIC]->texture, &uniforms);
	computePacks[ComputePack::NORMAL]->compute(textures[GPUTexture::DEPTH_METRIC_FILTERED]->texture, &uniforms);
}

void ElasticFusion::computeFramewiseLabels(const unsigned char * rgb, const unsigned short * depth)
{
	computeVertexesAndNormals();

	float * rawVertexes = new float[Resolution::getInstance().numPixels() * 3];
	float * rawNormals = new float[Resolution::getInstance().numPixels() * 3];

	float * tempVertexConf = new float[Resolution::getInstance().numPixels() * 4];
	float * tempNormalRad = new float[Resolution::getInstance().numPixels() * 4];

	float * projectedVertexes = new float[Resolution::getInstance().numPixels() * 3];
	float * projectedNormals = new float[Resolution::getInstance().numPixels() * 3];
	unsigned short * projectedLabels = new unsigned short[Resolution::getInstance().numPixels()];

	textures[GPUTexture::VERTEX_METRIC]->texture->Download(rawVertexes, GL_RGB, GL_FLOAT);
	textures[GPUTexture::NORMAL_METRIC_FILTERED]->texture->Download(rawNormals, GL_RGB, GL_FLOAT);

	indexMap.vertConfTex()->texture->Download(tempVertexConf, GL_RGBA, GL_FLOAT);
	indexMap.normalRadTex()->texture->Download(tempNormalRad, GL_RGBA, GL_FLOAT);
	indexMap.labelTex()->texture->Download(projectedLabels, GL_LUMINANCE_INTEGER_EXT, GL_UNSIGNED_SHORT);

	for (int i = 0; i < Resolution::getInstance().numPixels(); ++i)
	{
        projectedVertexes[3 * i] = tempVertexConf[4 * i];
        projectedVertexes[3 * i + 1] = tempVertexConf[4 * i + 1];
        projectedVertexes[3 * i + 2] = tempVertexConf[4 * i + 2];

        projectedNormals[3 * i] = tempNormalRad[4 * i];
        projectedNormals[3 * i + 1] = tempNormalRad[4 * i + 1];
		projectedNormals[3 * i + 2] = tempNormalRad[4 * i + 2];
	}

	delete [] tempVertexConf;
	delete [] tempNormalRad;

	frameSegmentation->setFrameData(rgb, depth, rawVertexes, rawNormals, projectedVertexes, projectedNormals, projectedLabels);

	frameSegmentation->process();

	textures[GPUTexture::LABEL]->texture->Upload(frameSegmentation->getPropagatedLabelMap(), GL_LUMINANCE_INTEGER_EXT, GL_UNSIGNED_SHORT);

	//unsigned short * mergingLabels = new unsigned short[2 * frameSegmentation->getMergingPairs().size()];
	//for (size_t i = 0; i < frameSegmentation->getMergingPairs().size(); ++i)
	//{
	//	mergingLabels[2 * i] = frameSegmentation->getMergingPairs()[i].first;
	//	mergingLabels[2 * i + 1] = frameSegmentation->getMergingPairs()[i].second;
	//}

	//textures[GPUTexture::MERGING_LABEL]->texture->Upload(mergingLabels, GL_LUMINANCE_ALPHA_INTEGER_EXT, GL_UNSIGNED_SHORT);

	delete [] rawVertexes;
	delete [] rawNormals;
	delete [] projectedVertexes;
	delete [] projectedNormals;
	delete [] projectedLabels;
	//delete [] mergingLabels;
}
...
void ElasticFusion::savePly()
{
    ...
    for(unsigned int i = 0; i < globalModel.lastCount(); i++)
    {
        ...
        if(pos[3] > confidenceThreshold)
        {
            ...
            unsigned char r = static_cast<int>(col[0]) >> 16 & 0xFF;
            unsigned char g = static_cast<int>(col[0]) >> 8 & 0xFF;
            unsigned char b = static_cast<int>(col[0]) & 0xFF;
...
void ElasticFusion::savePatchPly()
{
    std::string patchDir(getenv("HOME")); 
	patchDir += "/ProjectData/PatchData";

	std::map<int, std::vector<std::array<float, 10>>> patchPointData;
    ...
    for (unsigned int i = 0; i < globalModel.lastCount(); i++)
    {
        ...
        if (pos[3] > confidenceThreshold)
        {
			...
			if (std::isnan(nor[0]) || std::isnan(nor[1]) || std::isnan(nor[2]))
				continue;
			...
			if (1)//label != 0)
			{
            	...
            	pointData[3] = static_cast<int>(col[0]) >> 16 & 0xFF;
            	pointData[4] = static_cast<int>(col[0]) >> 8 & 0xFF;
            	pointData[5] = static_cast<int>(col[0]) & 0xFF;

				pointData[9] = nor[3];

				patchPointData[label].push_back(pointData);
			}
        }
    }
	...
	for (const auto & patch : patchPointData)
	{
		...
		if (!patchFile)
		{
			std::cerr << "Error: couldn't save patch ply." << std::endl;
		}

    	// Write header
		patchFile << "ply" << std::endl;
		patchFile << "format ascii 1.0" << std::endl;
		patchFile << "element vertex " << patch.second.size() << std::endl;
		patchFile << "property float x" << std::endl;
		patchFile << "property float y" << std::endl;
		patchFile << "property float z" << std::endl;
		patchFile << "property uchar red" << std::endl;
		patchFile << "property uchar green" << std::endl;
		patchFile << "property uchar blue" << std::endl;
		patchFile << "property float nx" << std::endl;
		patchFile << "property float ny" << std::endl;
		patchFile << "property float nz" << std::endl;
		patchFile << "property float radius" << std::endl;
		patchFile << "end_header" << std::endl;

		for (size_t i = 0; i < patch.second.size(); ++i)
		{
			patchFile << patch.second[i][0] << " ";
			patchFile << patch.second[i][1] << " ";
			patchFile << patch.second[i][2] << " ";

			patchFile << static_cast<int>(patch.second[i][3]) << " ";
			patchFile << static_cast<int>(patch.second[i][4]) << " ";
			patchFile << static_cast<int>(patch.second[i][5]) << " ";

			patchFile << patch.second[i][6] << " ";
			patchFile << patch.second[i][7] << " ";
			patchFile << patch.second[i][8] << " ";

			patchFile << patch.second[i][9] << std::endl;
		}
...
void ElasticFusion::mergeRecognizedPatches(const std::vector<std::vector<unsigned short>> & recognizedPatchLabels)
{
	for (auto sortedPatchLabels : recognizedPatchLabels)
	{
		std::sort(sortedPatchLabels.begin(), sortedPatchLabels.end());
		std::vector<std::pair<unsigned short, unsigned short>> mergingPairs;
		for (size_t i = 1; i < sortedPatchLabels.size(); ++i)
		{
			mergingPairs.emplace_back(sortedPatchLabels[0], sortedPatchLabels[i]);
		}

		frameSegmentation->insertMergingPairs(mergingPairs);
	}
}
...
unsigned short * ElasticFusion::getProjectedLabels()
{
	unsigned short * projectedLabels = new unsigned short[Resolution::getInstance().numPixels()];
	indexMap.labelTex()->texture->Download(projectedLabels, GL_LUMINANCE_INTEGER_EXT, GL_UNSIGNED_SHORT);
