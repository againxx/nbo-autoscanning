...
void main() 
{
    if(colorType0[0] != -1)
    {
		...
		{
			int labelConf = int(vColor[0].y) & 0xFF;
			int label = int(vColor[0].y) >> 8 & 0xFFFF;
			if (label == 65535)
			{
				vColor0.xyz = vec3(1, 1, 1);
			}
			else
			{
				if (labelConf > 0)
				{
					vColor0.x = float(label * 0x11 & 0xFF) / 255.0f;
					vColor0.y = float(label * 0x55 & 0xFF) / 255.0f;
					vColor0.z = float(label * 0xDD & 0xFF) / 255.0f;
				}
				else
				{
					vColor0.xyz = vec3(0, 0, 0);
				}
			}
		}
