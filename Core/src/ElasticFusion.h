...
class ElasticFusion
{
    public:
		...
		void mergeRecognizedPatches(const std::vector<std::vector<unsigned short>> & recognizedPatchLabels);

		void setGroundPlainLabels(const std::vector<unsigned short> & labels) { groundPlainLabels = labels; }

		unsigned short * getProjectedLabels();
    ...
    private:
		...
		void computeVertexesAndNormals();

		void computeFramewiseLabels(const unsigned char * rgb, const unsigned short * depth);
