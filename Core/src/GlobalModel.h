...
class GlobalModel
{
    public:
		...
		//void mergePatches(const std::vector<std::pair<unsigned short, unsigned short>> & mergingPairs);

        EFUSION_API unsigned int lastCount();

        Eigen::Vector4f * downloadMap();

		void uploadMap(const Eigen::Vector4f * vertices);

		void mergeLabelPairs(const std::vector<std::pair<unsigned short, unsigned short>> & labelPairs);

		std::set<unsigned short> getExistingLabels(unsigned short labelRange);

    private:
		float4 * getMappedPointer();
